...
class BerniniModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.config.mllm_config_path is not None:
            mllm_mmgp_path = os.path.join(self.config.mllm_config_path, self.config.mllm_subfolder or "", "model-mmgp.safetensors")
            if os.path.exists(mllm_mmgp_path):
                logger.info(f"Loading MLLM using mmgp from {mllm_mmgp_path}...")
                from mmgp import offload
                self.mllm = offload.fast_load_transformers_model(
                    mllm_mmgp_path,
                    do_quantize=False,
        ...
        if config.connector_cfg.get('enable_gen_branch', True):
            if self.diff_dec is None:
                logger.warning("self.diff_dec is None, but enable_gen_branch is True. This is normal during segmented mmgp loading.")
        if config.connector_cfg.get('enable_vit_branch', True):
            if self.mllm is None:
                logger.warning("self.mllm is None, but enable_vit_branch is True. This is normal during segmented mmgp loading.")
        ...
        if self.use_t5_encoder:
            ...
            def load_t5_text_encoder():
                ...
                if os.path.exists(t5_mmgp_path):
                    logger.info(f"Loading T5 Text Encoder using mmgp from {t5_mmgp_path}...")
                    from mmgp import offload
