...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.scale_shift_table.device != device or self.scale_shift_table.dtype != dtype:
            self.scale_shift_table.data = self.scale_shift_table.data.to(device=device, dtype=dtype)
        if hasattr(self.norm2, "weight") and getattr(self.norm2, "weight", None) is not None:
            if self.norm2.weight.device != device or self.norm2.weight.dtype != dtype:
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def patch_vae_latent(self, hidden_states: torch.Tensor, source_id=None):
        ...
        if next(self.patch_embedding.parameters()).device != hidden_states.device or next(self.patch_embedding.parameters()).dtype != hidden_states.dtype:
            self.patch_embedding.to(device=hidden_states.device, dtype=hidden_states.dtype)
    ...
    def forward(
        ...
        safe_to_device(self.condition_embedder)
        safe_to_device(self.norm_out)
        safe_to_device(self.proj_out)
        if self.scale_shift_table.device != device or self.scale_shift_table.dtype != dtype:
            self.scale_shift_table.data = self.scale_shift_table.data.to(device=device, dtype=dtype)
